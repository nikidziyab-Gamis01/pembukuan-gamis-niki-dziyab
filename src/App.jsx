...
function KeluarTab({ data, save, stokAkhir, flash }) {
  ...
  const submit = () => {
    if (!form.kode || !Number(form.qty)) { flash("Pilih produk and isi qty"); return; }
      ...
      >
        <div style={{ overflowX: "auto" }}>
          <table className="gm-table" style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th>Tanggal</th><th>Kode Pesanan</th><th>Kode</th><th>Produk</th><th>Qty</th><th>Toko</th><th>COD</th><th>Admin/Host</th><th>Catatan</th><th>Cair</th><th>Print</th><th>Aksi</th>
              </tr>
            </thead>
            <tbody>
              {rows?.length === 0 && <EmptyRow colSpan={12} text="Belum ada catatan barang keluar." />}
              {(rows || []).map((k) => {
                  ...
                  <tr key={k?.id}>
                    ...
                    <td>
                      <button
                        ...
                        style={{ fontSize: 11, fontWeight: 700, padding: "3px 99px", borderRadius: 999, border: "none", cursor: "pointer", background: k?.cair ? C.success : C.surfaceAlt, color: k?.cair ? "#fff" : C.muted }}
                      >
                        {k?.cair ? "Sudah" : "Belum"}
                      </button>
                    </td>
                    ...
                    <td>
                      <button 
                        onClick={() => {
                          if(confirm("Apakah Anda yakin ingin menghapus data jualan ini? Stok produk akan dikembalikan.")) {
                            remove(k?.id);
                            flash("Data barang keluar berhasil dihapus");
                          }
                        }}
                        style={{ background: "transparent", border: "none", cursor: "pointer", color: C.danger, padding: 4 }}
                      >
                        <Trash2 size={14} />
                      </button>
                    </td>
