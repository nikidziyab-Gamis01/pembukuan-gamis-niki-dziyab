...
function Dashboard({ data, stockMap, stokAkhir }) {
  ...
  const totalProduk = safeData.products.length;
  ...
  const cards = [
    ...
    { label: "Beban Gaji", value: fmt(totalBebanGaji), color: C.secondary },
  ];
  ...
  return (
    <div>
      ...
      >
        <div style={{ overflowX: "auto" }}>
          <table className="gm-table" style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr><th>Tanggal</th><th>Kode Pesanan</th><th>Kode</th><th>Produk</th><th>Qty</th><th>Toko</th><th>COD</th><th>Affiliate</th><th>Catatan</th><th>Cair</th><th>Cetak</th></tr></thead>
            <tbody>
...
function KeuanganTab({ data, save, flash }) {
  const [panel, setPanel] = useState("gaji-manual");
  const [formGaji, setFormGaji] = useState({
    gajiPokokOwner: data?.gajiSetelan?.gajiPokokOwner || 0,
    komisiOwnerPerPcs: data?.gajiSetelan?.komisiOwnerPerPcs || 0,
    komisiHostPerPcs: data?.gajiSetelan?.komisiHostPerPcs || 0,
    komisiAdminPerPcs: data?.gajiSetelan?.komisiAdminPerPcs || 0,
  });
  ...
  const submitGajiSetelan = () => {
    save({
      ...data,
      gajiSetelan: {
        gajiPokokOwner: Number(formGaji.gajiPokokOwner) || 0,
        komisiOwnerPerPcs: Number(formGaji.komisiOwnerPerPcs) || 0,
        komisiHostPerPcs: Number(formGaji.komisiHostPerPcs) || 0,
        komisiAdminPerPcs: Number(formGaji.komisiAdminPerPcs) || 0,
      },
    });
    flash("Pengaturan gaji berhasil disimpan");
  };
  ...
  const submitKasbon = () => {
    ...
    save({
      ...data,
      kasbon: [entry, ...(data?.kasbon || [])],
    });
    setFormKasbon({ tanggal: todayStr(), nama: "", nominal: "", keterangan: "" });
    flash("Kasbon dicatat");
  };

  const removeKasbon = (id) => {
    save({ ...data, kasbon: (data?.kasbon || []).filter((k) => k?.id !== id) });
  };

  // Hitung slip gaji
  const salarySlips = calculateSalarySlips(data);
  ...
  const printSlip = (slip) => {
    const html = `
      ...
      <body style="font-family: Arial; padding: 20px; max-width: 600px;">
        ...
        <hr/>
        <p><strong>Qty Barang Keluar:</strong> ${fmt(slip.qtyKeluar)}</p>
        <p><strong>Gaji Bruto:</strong> Rp ${fmt(slip.bruto)}</p>
        <p><strong>Potongan Kasbon:</strong> Rp ${fmt(slip.kasbon)}</p>
        <hr/>
        <p style="font-size: 18px;"><strong>Sisa Gaji Bersih: Rp ${fmt(slip.bersih)}</strong></p>
        <hr/>
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        <button
          ...
          style={{
            ...
            fontWeight: 600,
            cursor: "pointer",
            border: `2px solid ${panel === "gaji-manual" ? C.primary : C.border}`,
            background: panel === "gaji-manual" ? C.primary : "#fff",
            color: panel === "gaji-manual" ? "#fff" : C.text,
          }}
        >
      ...
      {/* PANEL 1: Pengaturan Gaji Manual */}
      {panel === "gaji-manual" && (
        <SectionCard title="Pengaturan Gaji Manual" subtitle="Atur komponen gaji karyawan">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14 }}>
            <Field label="Gaji Pokok Owner (Rp)">
              <input
                className="gm-input"
                type="number"
                min="0"
                value={formGaji.gajiPokokOwner}
                onChange={(e) => setFormGaji({ ...formGaji, gajiPokokOwner: e.target.value })}
              />
            </Field>
            <Field label="Komisi Owner per Pcs (Rp)">
              <input
                className="gm-input"
                type="number"
                min="0"
                value={formGaji.komisiOwnerPerPcs}
                onChange={(e) => setFormGaji({ ...formGaji, komisiOwnerPerPcs: e.target.value })}
              />
            </Field>
            <Field label="Komisi Host per Pcs (Rp)">
              <input
                className="gm-input"
                type="number"
                min="0"
                value={formGaji.komisiHostPerPcs}
                onChange={(e) => setFormGaji({ ...formGaji, komisiHostPerPcs: e.target.value })}
              />
            </Field>
            <Field label="Komisi Admin per Pcs (Rp)">
              <input
                className="gm-input"
                type="number"
                min="0"
                value={formGaji.komisiAdminPerPcs}
                onChange={(e) => setFormGaji({ ...formGaji, komisiAdminPerPcs: e.target.value })}
              />
            </Field>
          </div>
          <div style={{ marginTop: 16 }}>
            <button className="gm-btn" onClick={submitGajiSetelan}>
          ...
          <SectionCard title="Histori Kasbon">
            <div style={{ overflowX: "auto" }}>
              <table className="gm-table" style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {(data?.kasbon || []).length === 0 && <EmptyRow colSpan={5} text="Belum ada catatan kasbon." />}
                  {(data?.kasbon || []).map((k) => (
                    <tr key={k?.id}>
                      <td>{k?.tanggal}</td>
                      <td style={{ fontWeight: 500 }}>{k?.nama}</td>
                      <td style={{ fontWeight: 700, color: C.secondary }}>Rp {fmt(k?.nominal)}</td>
                      <td>{k?.keterangan}</td>
                      <td>
              ...
              <tbody>
                ...
                {salarySlips.map((slip) => (
                  <tr key={slip.nama}>
                    ...
                    <td style={{ fontWeight: 500 }}>{fmt(slip.qtyKeluar)}</td>
                    <td style={{ fontWeight: 600, color: C.primary }}>Rp {fmt(slip.bruto)}</td>
                    <td style={{ fontWeight: 600, color: C.danger }}>Rp {fmt(slip.kasbon)}</td>
                    <td style={{ fontWeight: 700, color: C.success }}>Rp {fmt(slip.bersih)}</td>
                    <td>
                      <button
...
function calculateSalarySlips(data) {
  ...
  // Kumpulkan semua karyawan dari keluar berdasarkan nama (host/admin)
  (data?.keluar || []).forEach((k) => {
    const hostName = String(k?.host || "").trim();
    const adminName = String(k?.shift || "").trim(); // Atau ambil dari field admin lain jika ada
    
    // Proses Host Live
    if (hostName) {
      if (!employeeMap[hostName]) {
        employeeMap[hostName] = {
          nama: hostName,
          role: "Host Live",
          qtyKeluar: 0,
          komisiPerPcs: Number(data?.gajiSetelan?.komisiHostPerPcs) || 0,
          isOwner: false,
        };
      }
      employeeMap[hostName].qtyKeluar += Number(k?.qty) || 0;
    }

    // Proses Admin Shift
    if (adminName && adminName !== hostName) {
      if (!employeeMap[adminName]) {
        employeeMap[adminName] = {
          nama: adminName,
          role: "Admin Shift",
          qtyKeluar: 0,
          komisiPerPcs: Number(data?.gajiSetelan?.komisiAdminPerPcs) || 0,
          isOwner: false,
        };
      }
      employeeMap[adminName].qtyKeluar += Number(k?.qty) || 0;
    }
  });

  // Tambahkan Owner secara default jika ada transaksi (asumsi ada owner)
  if ((data?.keluar || []).length > 0) {
    if (!employeeMap["Owner"]) {
      employeeMap["Owner"] = {
        nama: "Owner (Niki Dziyab)",
        role: "Owner",
        qtyKeluar: (data?.keluar || []).reduce((s, k) => s + (Number(k?.qty) || 0), 0),
        gajiPokok: Number(data?.gajiSetelan?.gajiPokokOwner) || 0,
        komisiPerPcs: Number(data?.gajiSetelan?.komisiOwnerPerPcs) || 0,
        isOwner: true,
      };
    }
  }
  ...
  Object.values(employeeMap).forEach((emp) => {
    const bruto = emp.isOwner
      ? emp.gajiPokok + emp.qtyKeluar * emp.komisiPerPcs
      : emp.qtyKeluar * emp.komisiPerPcs;
    ...
    slips.push({
      ...
      bruto,
      kasbon: kasbonTotal,
      bersih: Math.max(0, bersih), // Jangan negatif
    });
  });
...
function calculateTotalSalaryLoad(data) {
  ...
  return slips.reduce((total, slip) => total + slip.bruto, 0);
}
