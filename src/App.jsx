...
function buildRows(data, stockMap, stokAkhir) {
  const produk = data.products.map((p) => {
    ...
    return {
      "Kode Barang": p.kode, "Nama Barang": p.nama, "Jenis Barang": p.jenis, Brand: p.brand,
      Bahan: p.bahan, "Warna & Artikel": p.warna, Ukuran: p.ukuran, Vendor: p.vendor,
      "Total Masuk": s.masuk, Terjual: s.keluar, "Retur OK": s.returOk, Reject: s.reject,
      "Stok Akhir": stokAkhir(p.kode),
    };
  });
  const masuk = data.masuk.map((m) => ({
    Tanggal: m.tanggal, "Kode Barang": m.kode, "Nama Barang": (data.products.find((p) => p.kode === m.kode) || {}).nama || "",
    Qty: m.qty, Vendor: m.vendor, Catatan: m.catatan,
  }));
  ...
  const masterCode = [];
  Object.entries(data.masterCode).forEach(([kategori, list]) => {
    list.forEach((item) => masterCode.push({ Kategori: kategori.toUpperCase(), Kode: item.code, Nama: item.name }));
  });
...
export default function GamisInventoryApp() {
  ...
  const fileInputRef = useRef(null);
    ...
    setData(next); // Tetap update tampilan layar lokal dengan cepat
    
    try {
      // Karena Firestore membutuhkan data berupa objek murni, kita bungkus array data Anda
      // Gunakan setDoc dengan ID tetap agar data tim Anda saling menimpa secara realtime (tidak menumpuk duplikat)
      ...
      console.log("Data berhasil sinkron ke Firebase!");
    } catch (e) {
...
function Dashboard({ data, stockMap, stokAkhir }) {
  ...
  const lowStock = data.products.filter((p) => stokAkhir(p.kode) <= 10);
...
function ProdukTab({ data, save, stockMap, stokAkhir, flash }) {
  ...
  const codeOf = (list, name) => (list.find((x) => x.name === name) || {}).code || "";
  const previewKode =
  ...
  const addProduk = () => {
    ...
    if (data.products.some((p) => p.kode === previewKode)) { flash("Kode barang sudah ada"); return; }
    const nama = form.nama.trim() || `${form.brand} ${form.jenis} ${form.warna} ${form.ukuran} ${form.vendor}`.toUpperCase();
    const next = { ...data, products: [...data.products, { kode: previewKode, nama, jenis: form.jenis, brand: form.brand, bahan: form.bahan, warna: form.warna, ukuran: form.ukuran, vendor: form.vendor }] };
    if (Number(form.stokAwal) > 0) {
      next.masuk = [...next.masuk, { id: uid(), tanggal: todayStr(), kode: previewKode, qty: Number(form.stokAwal), vendor: form.vendor, catatan: "Stok awal" }];
    }
    save(next);
  ...
  const removeProduk = (kode) => {
    const hasLogs = [...data.masuk, ...data.keluar, ...data.retur, ...data.reject].some((r) => r.kode === kode);
    if (hasLogs) { flash("Tidak bisa dihapus — produk ini punya riwayat transaksi"); return; }
    save({ ...data, products: data.products.filter((p) => p.kode !== kode) });
  };

  const filtered = data.products.filter((p) => (p.kode + p.nama + p.warna + p.brand).toLowerCase().includes(q.toLowerCase()));
...
function MasterTab({ data, save, flash }) {
  ...
  const list = data.masterCode[cat];
