...
export default function GamisInventoryApp() {
  ...
  // 🔐 Daftar putih email tunggal yang diizinkan masuk ke aplikasi
  const whitelistEmails = [
    "nikidziyab@gmail.com"
  ];

  const isWhitelisted = (email) => {
    if (!email) return false;
    return whitelistEmails.map((e) => e.toLowerCase()).includes(email.toLowerCase());
  };
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
      ...
      const safeEmail = currentUser.email || currentUser.providerData?.[0]?.email || "";
      if (isWhitelisted(safeEmail)) {
        setUser(currentUser);
        setAuthLoading(false);
        return;
      }
