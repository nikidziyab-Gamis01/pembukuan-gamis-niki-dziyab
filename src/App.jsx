...
function ProdukTab({ data, save, stockMap, stokAkhir, flash }) {
  ...
  // KODE TERAKHIR DIJAMIN SINKRON DENGAN FIREBASE MAP
    const targetProducts = (data && data.products) ? data.products : (Array.isArray(data) ? data : []);
  
  const filteredProducts = targetProducts.filter((item) => {
    ...
    // PERBAIKAN FINAL: Menggunakan tanda tanya pengaman agar kebal jika variabel search tidak ada
    const kataKunci = String(typeof search !== 'undefined' ? search : (typeof searchTerm !== 'undefined' ? searchTerm : "")).toLowerCase();
