...
export default function GamisInventoryApp() {
  ...
  const fileInputRef = useRef(null);
    ...
    const unsubscribe = onSnapshot(doc(db, "pembukuan_gamis", "data_utama"), (snapshot) => {
      if (snapshot.exists()) {
        ...
        // Memasukkan data berupa objek Map {} agar sub-array di dalamnya terbaca sempurna
        setData(cloudData.pembukuanData || {});
      } else {
        // Jika dokumen belum ada, set dengan objek standar agar aplikasi tidak crash
        setData({
          products: [],
          masuk: [],
          keluar: [],
  ...
  const save = async (next) => {
    // 1. Update tampilan di layar laptop lokal dengan cepat
    setData(next);
