...
const normalizeData = (value) => {
  ...
  return {
    ...
    utangPiutang: Array.isArray(source.utangPiutang) ? source.utangPiutang : [], // <-- TAMBAHKAN BARIS INI AGAR DATABASE TIDAK MEMBUANG DATA UTANG
    masterCode: {
  ...
  return (
    <div style={{ background: C.bg, minHeight: 640, fontFamily: "'IBM Plex Sans', sans-serif", color: C.text }}>
      ...
      <div style={{ borderBottom: `1px solid ${C.border}`, background: C.surface, position: "sticky", top: 0, zIndex: 10 }}>
        <div style={{ padding: "10px 16px 0" }}> {/* Perkecil padding atas bawah */}
          ...
          <div className="gm-header-flex" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            ...
            <div className="gm-brand-group" style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 30, borderRadius: 8, background: C.primary, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, margin: "0 auto" }}>
                <Shirt size={16} color={C.accentSoft} />
              </div>
              <div>
                <div className="gm-h1" style={{ fontSize: 19, fontWeight: 700, lineHeight: 1.1, color: C.primaryDark }}>Pembukuan Gamis Niki Dziyab</div>
                {/* Diberi class gm-subtitle-hp agar otomatis hilang saat dibuka di HP */}
                <div className="gm-subtitle-hp" style={{ fontSize: 11.5, color: C.muted, marginTop: 2 }}>Stok & HPP Master · Barang Masuk & Keluar · Retur · Reject · Pencairan · Keuangan</div>
              </div>
            </div>
            ...
            <div className="gm-actions-group" style={{ display: "flex", alignItems: "center", gap: 6, position: "relative", flexWrap: "wrap", justifyContent: "flex-end" }}>
              ...
              <div className="gm-account-badge" style={{ display: "flex", alignItems: "center", gap: 6, background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: 999, padding: "4px 10px 4px 6px", marginRight: 4 }}>
                {user?.photoURL ? (
                  <img src={user.photoURL} alt="Avatar" style={{ width: 22, height: 26, borderRadius: "50%", objectFit: "cover", border: `1px solid ${C.border}` }} />
                ) : (
                  <div style={{ width: 22, height: 22, borderRadius: "50%", background: C.primary, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", lineHeight: 1.1, minWidth: 0, textAlign: "left" }}>
                  <span style={{ fontSize: 9, color: C.muted, fontWeight: 700, textTransform: "uppercase" }}>Akun Tim</span>
                  <span style={{ fontSize: 11, color: C.primaryDark, fontWeight: 600, maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {user?.displayName || user?.email || "Google User"}
                  </span>
                </div>
              </div>

              {/* BARISAN TOMBOL (OTOMATIS CONVERT MENJADI KUMPULAN ICON BULAT RAPI 1 BARIS DI HP) */}
              <button className="gm-btn-ghost gm-btn-wrap-hp" style={{ borderRadius: 8, padding: "8px 12px", fontSize: 12.5, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", background: "transparent" }} onClick={logout} title="Logout">
                <XCircle size={15} /> <span>Logout</span>
              </button>

              <input ref={fileInputRef} type="file" accept="application/json" onChange={handleImportFile} style={{ display: "none" }} />
              <button className="gm-btn-ghost gm-btn-wrap-hp" style={{ borderRadius: 8, padding: "8px 12px", fontSize: 12.5, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", background: "transparent" }} onClick={() => fileInputRef.current?.click()} title="Impor JSON">
                <Upload size={15} /> <span>Impor</span>
              </button>
              <button className="gm-btn-ghost gm-btn-wrap-hp" style={{ borderRadius: 8, padding: "8px 12px", fontSize: 12.5, fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer", background: "transparent" }} onClick={saveAsFile} title="Backup File">
                <Download size={15} /> <span>Backup File</span>
              </button>
              <button className="gm-btn gm-btn-wrap-hp" style={{ padding: "8px 14px", fontSize: 12.5, background: C.success }} onClick={syncCloudNow} title="Update Real-time">
                <Save size={15} /> <span>Real-time</span>
              </button>
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16, alignItems: "stretch", marginBottom: 18 }}>
        ...
        <SectionCard title="🔥 5 Produk Terlaris (Top Selling)" subtitle="Berdasarkan Kuantitas Unit Terjual">
          <div style={{ height: 220 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topProductsData} margin={{ top: 15, right: 8, left: -18, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke={C.border} vertical={false} />
                <XAxis dataKey="sku" tick={{ fontSize: 10, fontWeight: 700, fill: C.primaryDark }} axisLine={{ stroke: C.border }} tickLine={false} />
                <YAxis tick={{ fontSize: 12, fill: C.muted }} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={{ borderRadius: 8, border: `1px solid ${C.border}`, fontSize: 12 }} />
                <Bar dataKey="qty" fill={C.accent} radius={[4, 4, 0, 0]}>
                  {topProductsData.map((entry, i) => (
                    <Cell key={i} fill={i === 0 ? C.accent : C.accentSoft} /> 
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
    ...
    <div>
      <SectionCard title="Pencatatan Log Masuk Persediaan" subtitle="Kolom nominal HPP otomatis memuat harga modal master, namun tetap dapat diubah manual jika terjadi fluktuasi biaya produksi dari vendor">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr 1fr 1fr 1.5fr auto", gap: 10, alignItems: "end" }}>
          ...
          <Field label="HPP Masuk (Rp)"><input className="gm-input" type="number" value={form.hpp} onChange={(e) => setForm({ ...form, hpp: e.target.value })} placeholder="Bawaan Master" /></Field>
...
function KeluarTab({ data, save, stokAkhir, flash }) {
  ...
  // --- STATE BARU UNTUK KALKULATOR DISKON MULTI-PRODUCT ---
  const [totalNotaMarketplace, setTotalNotaMarketplace] = useState("");
  ...
  // Efek Otomatis: Menghitung harga per pcs saat total nota marketplace atau Qty berubah
  useEffect(() => {
    if (!isSampleCategory && totalNotaMarketplace && Number(form.qty) > 0) {
      const hargaRataRata = Math.round(Number(totalNotaMarketplace) / Number(form.qty));
      setForm((f) => ({ ...f, hargaJual: hargaRataRata }));
    }
  }, [totalNotaMarketplace, form.qty, isSampleCategory]);

  const submit = () => {
    if (!form.kode || !Number(form.qty)) { flash("Pilih produk dan isi qty keluar!"); return; }
    
    if (!isSampleCategory && !form.hargaJual) {
      flash("Mohon isi harga jual bruto atau gunakan kalkulator total nota!");
      return;
    }
    ...
    // Reset form dan kalkulator setelah simpan sukses
    setForm({ ...emptyForm, tanggal: form.tanggal });
    setTotalNotaMarketplace(""); 
    ...
    <div>
      <SectionCard title="Catat Jualan / Barang Keluar Omnichannel" subtitle="Rekam pengeluaran stok komersial maupun alokasi biaya pengiriman sampel kreator gratis">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12, marginBottom: 16 }}>
          ...
          <Field label="No. Pesanan / Resi">
            <input className="gm-input" value={form.kodePesanan} onChange={(e) => setForm({ ...form, kodePesanan: e.target.value })} placeholder="Resi / nama affiliate" />
          </Field>
        ...
        {/* BARIS INPUT BAWAH + MODUL KALKULATOR INTEGRASI */}
        <div style={{ display: "grid", gridTemplateColumns: "1.5fr 0.8fr 1.2fr 1fr 1.2fr", gap: 12, alignItems: "end" }}>
          ...
          {/* UTILITY 1: INPUT TOTAL NOTA MARKETPLACE */}
          <Field label="🧮 Total Nota Bersih (Setelah Diskon)">
            <input 
              className="gm-input" 
              style={{ background: isSampleCategory ? "#f3f4f6" : "#fff8f6", borderColor: C.secondary }} 
              type="number" 
              value={isSampleCategory ? "" : totalNotaMarketplace} 
              onChange={(e) => setTotalNotaMarketplace(e.target.value)} 
              placeholder="Ketik total harga resi..." 
              disabled={isSampleCategory}
            />
          </Field>
          
          {/* UTILITY 2: HARGA PER PCS OTOMATIS TER-ISI */}
          <Field label="Harga Jual per Pcs (Rp)">
            <input 
              className="gm-input" 
              type="number" 
              value={isSampleCategory ? 0 : form.hargaJual} 
              onChange={(e) => setForm({ ...form, hargaJual: e.target.value })} 
              placeholder={isSampleCategory ? "Otomatis Rp 0" : "Terisi otomatis..."} 
              disabled={isSampleCategory}
            />
          </Field>
...
function BukuKasTab({ data, save, flash }) {
  ...
  const handleTambahUP = () => {
    ...
    let currentUtangPiutang = Array.isArray(safeData.utangPiutang) ? safeData.utangPiutang : [];
    const nextUP = [entryUP, ...currentUtangPiutang];
    
    let nextBukuKas = [...(safeData.bukuKas || [])];
    
    // --- PERIKSA BAGIAN PENENTU DEBIT KREDIT INI ---
    if (formUP.jenis === "UTANG") {
      // Toko terima dana talangan dari luar = UANG MASUK TOKO (DEBIT)
      nextBukuKas.unshift({
        id: uid(),
        tanggal: formUP.tanggal,
        jenis: "Debit", // <-- PASTIKAN DI SINI TERTULIS "Debit"
        nominal: nominalAngka,
        keterangan: `[Pencatatan Utang] Terima dana talangan pribadi dari ${formUP.pihak.toUpperCase()} | Keterangan: ${formUP.keterangan}`
      });
    } else {
      // Toko meminjamkan kas toko ke orang luar = UANG KELUAR TOKO (KREDIT)
      nextBukuKas.unshift({
        id: uid(),
        tanggal: formUP.tanggal,
        jenis: "Kredit", // <-- PASTIKAN DI SINI TERTULIS "Kredit"
        nominal: nominalAngka,
        keterangan: `[Pencatatan Piutang] Dana toko dipinjam oleh ${formUP.pihak.toUpperCase()} | Keterangan: ${formUP.keterangan}`
      });
    }
  ...
  const removeJurnal = (id) => {
    if (confirm("Hapus catatan jurnal kas ini?")) {
      const itemKas = (safeData.bukuKas || []).find((b) => b.id === id);
      let nextUtangPiutang = [...(safeData.utangPiutang || [])];
      
      // Jika jurnal kas yang dihapus mengandung info nama pihak utang, hapus juga log utangnya agar sinkron kembali
      if (itemKas && itemKas.keterangan) {
        nextUtangPiutang = nextUtangPiutang.filter((up) => 
          !itemKas.keterangan.includes(up.pihak)
        );
      }

      save({ 
        ...safeData, 
        bukuKas: (safeData.bukuKas || []).filter((b) => b.id !== id),
        utangPiutang: nextUtangPiutang
      });
      flash("Jurnal & sinkronisasi data terkait berhasil dihapus!");
    }
  };
...
// ---------- KODE UTUH: Keuangan & Gaji Tim (Atur Gaji, Kasbon + Hapus, Slip Gaji) ----------
function KeuanganTab({ data, save, flash }) {
  ...
  const rekapSlips = useMemo(() => {
    ...
    (safeData.karyawanMaster || []).forEach((e) => {
      empMap[e.nama] = { id: e.id, nama: e.nama, role: e.peran || "Tim Live", qtyKeluar: 0 };
    });
        ...
        <div>
          <SectionCard title="Pencatatan Ambil Dana Pinjaman Kasbon Karyawan">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr 1fr 2fr auto", gap: 10, alignItems: "end" }}>
              <ThemeField label="Tanggal"><input className="gm-input" type="date" value={formKasbon.tanggal} onChange={(e) => setFormKasbon({ ...formKasbon, tanggal: e.target.value })} /></ThemeField>
              <ThemeField label="Nama Pegawai">
                <select className="gm-select" value={formKasbon.nama} onChange={(e) => setFormKasbon({ ...formKasbon, nama: e.target.value })}>
                  <option value="">Pilih Pegawai…</option>
                  {karyawanList.map((k) => <option key={k} value={k}>{k}</option>)}
                </select>
              </ThemeField>
              <ThemeField label="Nominal Pinjaman"><input className="gm-input" type="number" value={formKasbon.nominal} onChange={(e) => setFormKasbon({ ...formKasbon, nominal: e.target.value })} /></ThemeField>
              <ThemeField label="Memo / Alasan"><input className="gm-input" value={formKasbon.keterangan} onChange={(e) => setFormKasbon({ ...formKasbon, keterangan: e.target.value })} placeholder="Bensin live / makan" /></ThemeField>
          ...
          <SectionCard title="Histori Log Pinjaman Kasbon">
            <div style={{ overflowX: "auto" }}>
              <table className="gm-table" style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    <th>Tanggal</th><th>Nama Pegawai</th><th>Nominal Pinjaman</th><th>Keterangan</th><th>Aksi</th>
                  </tr>
                </thead>
                <tbody>
                  {(safeData.kasbon || []).length === 0 && <EmptyRow colSpan={5} text="Belum ada catatan kasbon pegawai." />}
                  {(safeData.kasbon || []).map((k) => (
                    <tr key={k.id}>
                      <td>{k.tanggal}</td>
                      <td><strong>{k.nama}</strong></td>
                      <td style={{ color: C.danger, fontWeight: "bold" }}>Rp {fmt(k.nominal)}</td>
                      <td style={{ whiteSpace: "normal" }}>{k.keterangan || "—"}</td>
                      <td>
                        <DeleteBtn onClick={() => {
                          if (confirm(`Hapus catatan kasbon ${k.nama} sebesar Rp ${fmt(k.nominal)}?\n\nCatatan: Penghapusan ini otomatis membuang log pengeluaran saldo pasangannya di Buku Kas Utama.`)) {
                            const nextKasbon = (safeData.kasbon || []).filter((x) => x.id !== k.id);
                            const nextBukuKas = (safeData.bukuKas || []).filter((b) => 
                              !(b.nominal === k.nominal && b.keterangan.includes(k.nama))
                            );
                            save({ ...safeData, kasbon: nextKasbon, bukuKas: nextBukuKas });
                            flash(`Kasbon ${k.nama} berhasil dihapus!`);
                          }
                        }} />
                      </td>
                    </tr>
        ...
        <SectionCard title="Rekap Slip Gaji Karyawan Otomatis" subtitle="Kalkulasi upah bruto dihitung terintegrasi dari gabungan akumulasi volume jualan live/shift tim">
          <div style={{ overflowX: "auto" }}>
            <table className="gm-table" style={{ width: "100%" }}>
              <thead>
                <tr>
                  <th>Nama Pegawai</th><th>Tugas Divisi</th><th>Total Jualan</th><th>Gaji Bruto</th><th>Potongan Kasbon</th><th>Sisa Gaji Bersih</th><th>Aksi Nota</th>
                </tr>
              </thead>
              <tbody>
                {rekapSlips.length === 0 && <EmptyRow colSpan={7} text="Belum ada rekap data tim operasional." />}
                {rekapSlips.map((s) => (
                  <tr key={s.nama}>
                    ...
                    <td>
                      {/* TOMBOL CETAK SLIP GAJI DIGITAL */}
                      <button 
                        className="gm-btn gm-btn-ghost" 
                        style={{ padding: "4px 8px", fontSize: "11px", borderRadius: "6px" }}
                        onClick={() => {
                          alert(`=== NOTA DIGITAL SLIP GAJI TIM ===\n\nNama Staf: ${s.nama}\nJabatan: ${s.role}\nTotal Closingan Live: ${fmt(s.qtyKeluar)} Pcs\n\n(+) Gaji Pokok + Insentif: Rp ${fmt(s.bruto)}\n(-) Total Potongan Kasbon: Rp ${fmt(s.kasbon)}\n===============================\n(=) TOTAL DITERIMA BERSIH: Rp ${fmt(s.bersih)}\n\nNota siap ditransfer atau dicetak via PDF utama.`);
                        }}
                      >
                        📄 Lihat Nota
                      </button>
...
// Komponen Pembantu Pengaman Label Bidang Formulir Internal
