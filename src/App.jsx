...
function BukuKasTab({ data, save, flash }) {
  ...
  const bukuKasList = (safeData?.bukuKas || [])
    .sort((a, b) => new Date(b?.tanggal) - new Date(a?.tanggal))
    .map((b) => {
      const isDebit = b?.jenis === "Debit";
      const nominalItem = Number(b?.nominal) || 0;
      if (isDebit) {
        totalDebit += nominalItem;
        saldoKumulatif += nominalItem;
      } else {
        totalKredit += nominalItem;
        saldoKumulatif -= nominalItem;
      }
      return {
        ...b,
  ...
  // Re-calculate saldo untuk setiap row (dari newest ke oldest)
  let runingSaldo = 0;
  const bukuKasWithSaldo = bukuKasList.map((b, idx) => {
    if (idx === 0) {
      runingSaldo = b?.jenis === "Debit" ? b?.nominal : -b?.nominal;
    } else {
      const prev = bukuKasWithSaldo[idx - 1];
      if (b?.jenis === "Debit") {
        runingSaldo = (prev?.saldoKumulatif || 0) + (b?.nominal || 0);
      } else {
        runingSaldo = (prev?.saldoKumulatif || 0) - (b?.nominal || 0);
      }
    }
    return { ...b, saldoKumulatif: runingSaldo };
  });

  const saldoAkhir = runingSaldo || 0;
