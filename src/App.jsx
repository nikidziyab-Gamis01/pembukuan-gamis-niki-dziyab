  ...
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginBottom: 18 }}>
        {cards.map((c) => (
          <div key={c.label} className="gm-card" style={{ padding: 16 }}>
            ...
            <div className="gm-h1" style={{ fontSize: 24, fontWeight: 700, color: c.color, marginTop: 6 }}>{c.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 16, alignItems: "stretch" }}>
          ...
          <div style={{ height: 220 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 4, right: 8, left: -18, bottom: 0 }}>
                ...
                <Bar dataKey="qty" fill={C.accentSoft} radius={[5, 5, 0, 0]}>
        ...
        <SectionCard title="Ringkasan Dana Cair" subtitle="Total volume keluar vs dana riil cair masuk kas">
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <Stat label="Total Volume Keluar" value={`${fmt(totalKeluar)} Pcs`} />
            <Stat label="Total Dana Cair Riil" value={`Rp ${fmt(totalCair)}`} color={C.success} />
          </div>
        </SectionCard>
      </div>
      ...
      <SectionCard title="Stok Menipis" subtitle="Produk dengan sisa unit ≤ 10" right={lowStock.length > 0 && <AlertTriangle size={16} color={C.danger} />}>
        {lowStock.length === 0 ? (
          <div style={{ fontSize: 13, color: C.muted }}>Seluruh stok aman berada di atas batas aman minimum.</div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {lowStock.map((p) => (
              <div key={p.kode} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", background: C.surfaceAlt, borderRadius: 8 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: C.danger }}>{stokAkhir(p.kode)} Pcs</span>
              </div>
            ))}
          </div>
        )}
...
// ---------- Barang Keluar (Multiplatform & Pencairan Manual) ----------
function KeluarTab({ data, save, stokAkhir, flash }) {
  ...
  const submit = () => {
    if (!form.kode || !Number(form.qty) || !form.hargaJual) { flash("Pilih produk, isi qty dan harga jual platform!"); return; }
    const sisa = stokAkhir(form.kode);
    if (Number(form.qty) > sisa) { flash(`Stok tidak cukup (tersisa ${sisa} Pcs)`); return; }
    
    const entry = { id: uid(), ...form, qty: Number(form.qty), hargaJual: Number(form.hargaJual), cair: false, nominalCairRiil: 0 };
  ...
  return (
    <div>
      <SectionCard title="Catat Jualan / Barang Keluar Omnichannel" subtitle="Rekam harga bruto aplikasi per channel marketplace untuk analisis omset">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px,1fr))", gap: 10 }}>
          <Field label="Tanggal Jual"><input className="gm-input" type="date" value={form.tanggal} onChange={(e) => setForm({ ...form, tanggal: e.target.value })} /></Field>
          <Field label="Kode Pesanan / Resi"><input className="gm-input" value={form.kodePesanan} onChange={(e) => setForm({ ...form, kodePesanan: e.target.value })} placeholder="Resi orderan" /></Field>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr auto", gap: 10, marginTop: 10, alignItems: "end" }}>
          ...
          <Field label="Harga Jual Bruto Aplikasi (Rp)"><input className="gm-input" type="number" value={form.hargaJual} onChange={(e) => setForm({ ...form, hargaJual: e.target.value })} placeholder="Harga per pcs" /></Field>
          <button className="gm-btn" onClick={submit}><Plus size={15} /> Simpan Order</button>
        </div>
      </SectionCard>

      <SectionCard title="Riwayat Barang Keluar & Buku Kas Mandiri" subtitle={`Volume jualan terfilter: ${fmt(totalQty)} Pcs`} right={
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="gm-table" style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th>Tanggal</th><th>Platform</th><th>Invoice / Resi</th><th>SKU</th><th>Qty</th><th>Omset Bruto</th><th>Input Bersih Cair</th><th>Konfirmasi Status Kas</th><th>Tindakan</th>
              </tr>
            </thead>
            <tbody>
              ...
              {rows.map((k) => {
                ...
                return (
                  <tr key={k.id}>
                    ...
                    <td>{fmt(k.qty)} Pcs</td>
                    <td>Rp {fmt(k.qty * (k.hargaJual || 0))}</td>
                    <td>
                      {!k.cair ? (
                        <input 
                          type="number" 
                    ...
                    <td>
                      <button
                        onClick={() => {
                          ...
                          flash("Mutasi nominal bersih sukses disinkronkan ke Buku Kas!");
                        }}
                      ...
                      >
                        {k.cair ? "✓ Cair Masuk Kas" : "Konfirmasi Dana"}
                      </button>
                    </td>
...
function ReturTab({ data, save, flash }) {
  ...
  const submit = () => {
    ...
    flash("Klaim Retur Berhasil Rekam");
  };
...
function KeuanganTab({ data, save, flash }) {
  ...
  const submitKasbon = () => {
    if (!formKasbon.nama || !Number(formKasbon.nominal)) { flash("Isi nama karyawan dan nominal kasbon"); return; }
    
    const entry = { id: uid(), ...formKasbon, nominal: Number(formKasbon.nominal) };
    save({
      ...safeData,
      ...
      bukuKas: [{
        ...
        keterangan: `[Pinjaman Kasbon Pegawai] Nama: ${formKasbon.nama} | Memo: ${formKasbon.keterangan || "Operasional"}`,
      }, ...(safeData.bukuKas || [])]
    });
  ...
  return (
    <div>
      <SectionCard title="Manajemen Kasbon Pegawai" subtitle="Setiap pengeluaran kasbon otomatis mengurangi posisi saldo di Master Buku Kas">
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr 1fr 2fr auto", gap: 10, alignItems: "end" }}>
          <Field label="Tanggal Ambil"><input className="gm-input" type="date" value={formKasbon.tanggal} onChange={(e) => setFormKasbon({ ...formKasbon, tanggal: e.target.value })} /></Field>
          <Field label="Nama Personel Pegawai">
            <select className="gm-select" value={formKasbon.nama} onChange={(e) => setFormKasbon({ ...formKasbon, nama: e.target.value })}>
              <option value="">Pilih Pegawai…</option>
              {karyawanList.map((k) => <option key={k} value={k}>{k}</option>)}
            </select>
          </Field>
          <Field label="Nominal Pinjaman (Rp)"><input className="gm-input" type="number" value={formKasbon.nominal} onChange={(e) => setFormKasbon({ ...formKasbon, nominal: e.target.value })} /></Field>
          <Field label="Alasan Keperluan"><input className="gm-input" value={formKasbon.keterangan} onChange={(e) => setFormKasbon({ ...formKasbon, keterangan: e.target.value })} /></Field>
  ...
  return (
    <div style={{ padding: 30, background: "#fff" }}>
      <div className="no-print" style={{ marginBottom: 20 }}>
        <button className="gm-btn" onClick={onClose}>← Kembali</button>
      </div>
